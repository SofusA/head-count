...
export class Database {
    ...
    private supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlhdCI6MTYyNjQyNzkyOCwiZXhwIjoxOTQyMDAzOTI4fQ.XoT5qhcfNYapF_rDJxzokPX9eOGCB0hzBW5crHvcnoA'
    ...
    subscribe(fn: () => void) {
        ...
        const sensor = ((this.loc === '*') ? this.supabase.from(`counter`).on('*', payload => { fn() }).subscribe() : this.supabase.from(`counter:location=eq.${this.loc}`).on('*', payload => { fn() }).subscribe())
    }

    getSensorList = async (): Promise<Array<Object>> => {
        // If location is * get all, else get from location
        const { data: sensors, error } = ((this.loc === '*') ? await this.supabase.from('sensor').select('*') : await this.supabase.from('sensor').select('*').eq('location', this.loc))
    ...
    currentVisitors = async (): Promise<Number> => {
        ...
        if (parseInt(DateTime.now().toFormat('H')) > 0 && parseInt(DateTime.now().toFormat('H')) < 3) {
            time = DateTime.now().plus({ days: -1 }).startOf('day').plus({ hours: 3 }).toMillis()
        } else {
